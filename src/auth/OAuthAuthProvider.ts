...
const ENV_CLIENT_ID = "ISLO_API_KEY" as const;
...
export class OAuthAuthProvider implements core.AuthProvider {
    ...
    private async refresh({ endpointMetadata }: { endpointMetadata?: core.EndpointMetadata } = {}): Promise<string> {
        this.refreshPromise = (async () => {
            try {
                const _clientId = await this.clientIdSupplier({ endpointMetadata });
                const clientSecret = await this.clientSecretSupplier({ endpointMetadata });
                const tokenResponse = await this.authClient.exchange_access_key({
                    access_key: clientSecret,
                });

                this.accessToken = tokenResponse.session_token;
...
export namespace OAuthAuthProvider {
    export const AUTH_SCHEME = "IsloAccessKey" as const;
